...
out = Path(r"d:\codespace\rentSystem\db_update_building9_result.json")
...
if "building" not in columns:
    ...
    cur.execute("SELECT COUNT(*) FROM rooms WHERE TRIM(CAST(building AS TEXT))='9'")
    before = cur.fetchone()[0]

    cur.execute(
        "UPDATE rooms SET building=? WHERE TRIM(CAST(building AS TEXT))='9'",
        ("登新公寓 9幢",),
    )
    ...
    cur.execute("SELECT COUNT(*) FROM rooms WHERE building=?", ("登新公寓 9幢",))
